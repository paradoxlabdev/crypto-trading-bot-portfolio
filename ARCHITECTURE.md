...
**Performance:** sub-millisecond in-memory lookup, high hit rate on channel metadata in normal operation.
...
**Performance:** single-digit millisecond Redis lookup, high hit rate for repeat-seen tokens.
...
**Performance:** single-digit millisecond Redis lookup. Caching this layer meant the external token API stopped being the bottleneck during processing bursts.
...
**Effect:** Redis outages below the circuit-breaker threshold degrade gracefully — reads fall back to the runtime cache, writes buffer or skip non-critical data, reconnect is transparent.
...
**Effect:** the client-side ceiling stays below Telegram's enforcement threshold, so rate-limit bans aren't a practical concern.
...
Order-of-magnitude, based on what the system handled during the engagement (exact figures not published here — internal dashboards I don't have access to post-ship):

- Users: hundreds of concurrent
- Tokens: tens of thousands per day
...
**Current:** Multi-layer cache (high hit rate on channel metadata in steady state)
